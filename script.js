...
function mergeUserData(local, cloud) {
  ...
  // Merge workouts (cloud wins for newer dates)
  if (cloud.workouts) {
    if (!merged.workouts) merged.workouts = {};
    Object.keys(cloud.workouts).forEach(date => {
      if (!merged.workouts[date]) {
        merged.workouts[date] = cloud.workouts[date];
      }
    });
  }
  ...
  if (cloud.meals) {
    ...
    Object.keys(cloud.meals).forEach(date => {
      if (!merged.meals[date]) {
        merged.meals[date] = cloud.meals[date];
      }
    });
  }
...
function debouncedSync() {
  ...
  }, 15000); // Batch changes into one write to stay gentle on the Supabase free plan.
}
